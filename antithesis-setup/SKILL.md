...
- Build and test locally with the compose CLI and container runtime `snouty
  doctor` reports. See `references/submit-and-test.md`.
...
- `snouty validate` on `antithesis/config/` succeeds
- All built images target `amd64` (verified via `image inspect` on the engine snouty selects)
- The build used the compose CLI and engine `snouty doctor` reports
